...
%		in 2D, Bx=B(:,:,1); By=B(:,:,2);
%   !!!B attention about B, its directions indeed are y,x,z which means you
%   need to use targetX=x+B(~,2), targetY=y+B(~,1), targetZ=z+B(~,3)
...
if(~strcmpi(Options.Registration(1),'N'))
    MASKmovingAf = true(size(MASKmoving));
    MASKstaticAf = true(size(MASKstatic));
    M=affine_registration(O_trans,Spacing,Options,Imoving,Istatic,MASKmovingAf,MASKstaticAf,type,Points1,Points2,PStrength,IS3D);
    %     [optimizer, metric] = imregconfig('monomodal');
    %     tform = imregtform(Imoving,Istatic,'affine',optimizer,metric);
    %     M = tform.T';
else M=[];
end
%Ireg = [];B = [];return;
...
resize3rdFlag = true;
if size(ISmoving,3) < 50 || size(ISstatic,3) < 50 % the 3rd dimension is small
    resize3rdFlag = false;
end
for refine_itt=1:3
    if(refine_itt==1)
        if(IS3D)
            if resize3rdFlag
                ...
                tmpISMov = cell(size(ISmoving,3), 1);
                for ii = 1:size(ISmoving,3)
                    tmpISMov{ii} = imresize(ISmoving(:,:,ii),0.25);
                end
                ITmoving = cat(3, tmpISMov{:});
                tmpISSta = cell(size(ISstatic,3), 1);
                for ii = 1:size(ISstatic,3)
                    tmpISSta{ii} = imresize(ISstatic(:,:,ii),0.25);
                end
                ITstatic = cat(3, tmpISSta{:});
            end
        else
        ...
        if(IS3D)
            if(~isempty(MASKmoving)) 
                if resize3rdFlag
                    ITMASKmoving = imresize3d(MASKmoving,0.25); 
                else
                    tmpISMov = cell(size(MASKmoving,3), 1);
                    for ii = 1:size(MASKmoving,3)
                        tmpISMov{ii} = imresize(MASKmoving(:,:,ii),0.25);
                    end
                    ITMASKmoving = cat(3, tmpISMov{:});
                end
                
            else
                ITMASKmoving=[];
            end
            if(~isempty(MASKstatic))
                if resize3rdFlag
                    ITMASKstatic = imresize3d(MASKstatic,0.25);
                else
                    tmpISMov = cell(size(MASKstatic,3), 1);
                    for ii = 1:size(MASKstatic,3)
                        tmpISMov{ii} = imresize(MASKstatic(:,:,ii),0.25);
                    end
                    ITMASKstatic = cat(3, tmpISMov{:});
                end
            else
                ITMASKstatic=[]; 
            end
    ...
    elseif(refine_itt==2)
        ...
        if(IS3D)
            if resize3rdFlag
                ...
                tmpISMov = cell(size(ISmoving,3), 1);
                for ii = 1:size(ISmoving,3)
                    tmpISMov{ii} = imresize(ISmoving(:,:,ii),0.5);
                end
                ITmoving = cat(3, tmpISMov{:});
                tmpISSta = cell(size(ISstatic,3), 1);
                for ii = 1:size(ISstatic,3)
                    tmpISSta{ii} = imresize(ISstatic(:,:,ii),0.5);
                end
                ITstatic = cat(3, tmpISSta{:});
            end
        else
        ...
        if(IS3D)
            if(~isempty(MASKmoving))
                if resize3rdFlag
                    ITMASKmoving = imresize3d(MASKmoving,0.5);
                else
                    tmpISMov = cell(size(MASKmoving,3), 1);
                    for ii = 1:size(MASKmoving,3)
                        tmpISMov{ii} = imresize(MASKmoving(:,:,ii),0.5);
                    end
                    ITMASKmoving = cat(3, tmpISMov{:});
                end
            ...
            if(~isempty(MASKstatic))
                if resize3rdFlag
                    ITMASKstatic = imresize3d(MASKstatic,0.5);
                else
                    tmpISMov = cell(size(MASKstatic,3), 1);
                    for ii = 1:size(MASKstatic,3)
                        tmpISMov{ii} = imresize(MASKstatic(:,:,ii),0.5);
                    end
                    ITMASKstatic = cat(3, tmpISMov{:});
                end
            else
    ...
    if(IS3D)
        optim=struct('GradObj','on','GoalsExactAchieve',1,'Display','off','StoreN',10,'HessUpdate','lbfgs','MaxIter',200,'MaxFunEvals',1000,'TolFun',1e-7,'DiffMinChange',1e-5);
    else
        optim=struct('GradObj','on','GoalsExactAchieve',1,'Display','off','StoreN',10,'HessUpdate','lbfgs','MaxIter',200,'MaxFunEvals',1000,'TolFun',1e-7,'DiffMinChange',1e-3);
    end
...
% Use struct because expanded optimset is part of the Optimization Toolbox.
if(IS3D)
    optim=struct('GradObj','on','GoalsExactAchieve',0,'StoreN',10,'HessUpdate','lbfgs','Display','off','MaxIter',20,'DiffMinChange',0.03,'DiffMaxChange',1,'MaxFunEvals',1000,'TolX',0.1,'TolFun',1e-5);
else
